...
//Этот ужасный рейтинг и звезды

const ratingItems = document.querySelectorAll('.simple-raiting__item'); // Получаю элемнты рейтинга и кнопку
const submitButton = document.querySelector('.form__btn');

const handleRatingClick = (event) => {
    const ratingValue = event.target.value; // Получаю значение рейтинга
    localStorage.setItem('rating', ratingValue); // Сохраняю значение в localStorage
};

ratingItems.forEach((item) => {// Присваиваю обработчик клика каждому элементу рейтинга
    item.addEventListener('click', handleRatingClick);
});

const savedRating = localStorage.getItem('rating'); // При загрузке страницы проверяю, есть ли сохраненное значение рейтинга в localStorage
if (savedRating) {
    const matchingRatingItem = document.querySelector(`.simple-raiting__item[value="${savedRating}"]`); // Устанавливаю сохраненное значение рейтинга
    if (matchingRatingItem) {
        matchingRatingItem.checked = true;
    }
}

// Выожу данные рейтинга в консоль
